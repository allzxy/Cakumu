...
function DeltaTag({ value, invert = false, compareLabel }: { value: number | null; invert?: boolean; compareLabel: string }) {
  const { t } = useLanguage();
  if (value === null || !isFinite(value)) return <span className="text-xs text-[var(--color-muted)]">{t('summary.noPriorData', { compare: compareLabel })}</span>;
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-semibold ${positive ? 'text-[var(--color-primary)]' : 'text-[var(--color-warn)]'}`}>
      ...
      {Math.abs(value).toFixed(1)}% {t('summary.vsLastMonth')}
    </span>
  );
}
...
export default function SummaryCards({ income, spending, balance, incomeDelta, spendingDelta }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
      {cards.map((card, i) => (
        <div key={card.label} className="animate-rise rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-flat)] sm:p-5" style={{ animationDelay: `${i * 80}ms` }}>
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate text-xs font-medium uppercase tracking-wide text-[var(--color-muted)]">{card.label}</p>
              <p className="font-bold tracking-tight mt-1.5 truncate text-2xl text-[var(--color-ink)] sm:mt-2 sm:text-3xl"><AnimatedAmount value={card.value} currency={currency} /></p>
            </div>
            <div className="animate-pop flex h-9 w-9 shrink-0 items-center justify-center rounded-xl sm:h-10 sm:w-10" style={{ backgroundColor: card.bg, color: card.accent, animationDelay: `${i * 80 + 120}ms` }}><card.icon size={17} strokeWidth={2.2} /></div>
          </div>
