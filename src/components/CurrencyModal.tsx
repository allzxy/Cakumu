...
export default function CurrencyModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t('currency.title')}>
      <div className="flex flex-col gap-5">
        <div
          className={`flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs ${
            liveRates.isLive ? 'bg-[var(--color-primary-soft)] text-[var(--color-primary-strong)]' : 'bg-[var(--color-warn-soft)] text-[var(--color-warn)]'
          }`}
        >
          {liveRates.status === 'loading' ? <RefreshCw size={13} className="shrink-0 animate-spin" /> : liveRates.isLive ? <Radio size={13} className="shrink-0" /> : <WifiOff size={13} className="shrink-0" />}
          <span className="min-w-0 flex-1 truncate">
            {liveRates.status === 'loading' ? t('currency.loading') : liveRates.isLive ? t('currency.liveOn', { time: fetchedTime }) : t('currency.liveOff')}
          </span>
          <button
            type="button"
        ...
        <div>
          <p className="mb-2 text-xs font-medium text-[var(--color-muted)]">{t('currency.displayLabel')}</p>
            ...
            {CURRENCIES.map((c) => (
              <button
                ...
                className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${
                  currencyCode === c.code ? 'bg-[var(--color-primary-soft)] text-[var(--color-primary-strong)]' : 'text-[var(--color-ink-soft)] hover:bg-[var(--color-surface-alt)]'
                }`}
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--color-surface-alt)] text-sm font-semibold">{c.symbol}</span>
        ...
        <div className="border-t border-[var(--color-border)] pt-4">
          <p className="mb-2 flex items-center gap-1.5 text-xs font-medium text-[var(--color-muted)]">
            <ArrowLeftRight size={12} /> {t('currency.quickConvert')}
          </p>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1.5">
              <SelectField nested compact modalTitle={t('currency.title')} value={fromCode} onChange={setFromCode} options={currencyOptions} />
              <button
                type="button"
                onClick={swap}
              ...
              <SelectField nested compact modalTitle={t('currency.title')} value={toCode} onChange={setToCode} options={currencyOptions} />
            </div>

            <div className="min-w-0 rounded-xl bg-[var(--color-primary-soft)] px-4 py-3">
              <p className="font-bold tracking-tight truncate text-xl text-[var(--color-primary-strong)]">{formatMoney(result, toCurrency)}</p>
