  ...
  return (
    <>
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate">{formatDisplay(value, placeholder ?? t('date.pick'), locale)}</span>
      </button>

      <DatePickerModal open={open} onClose={() => setOpen(false)} value={value} onChange={onChange} nested={nested} allowClear={allowClear} />
    </>
  );
}
